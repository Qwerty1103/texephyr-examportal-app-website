...
const EventForm = (props) => {
  ...
  const [feesM, setFeesM] = useState("0");
  const [feesN, setFeesN] = useState("0");
  ...
  const feesMHandler = (event) => {
    setFeesM(event.target.value);
  };
  const feesNHandler = (event) => {
    setFeesN(event.target.value);
  };
  ...
  const submitHandler = (event) => {
    ...
    formData.set('mitfees', feesM)
    formData.set('otherfees', feesN)
    ...
    <form id="eventForm" onSubmit={submitHandler}>
      <div className="eventAdder__container__eventForm swing-in-top-fwd ">
        ...
        <div className="eventAdder__container__eventForm-groupD">
          ...
          <div>
            <label>Fees(Non-Mitians)</label>
            <input
              type="number"
              value={feesN}
              onChange={feesNHandler}
              required
            />
          </div>
        </div>
