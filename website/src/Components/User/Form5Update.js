...
const Form5Update = (props) => {
  const [id, setid] = useState({ idArray: ["", "", "", ""] })
  const [checkmarks, setCheckMarks] = useState(["hide", "hide", "hide", "hide"])
  const [disableBtn, setDisableBtn] = useState(true);
  const [count, setCount] = useState({ countArray: [1, 1, 1, 1] })
  ...
  useEffect(() => {
    axios(
        {
          method: "POST",
          url: USERAPIURL + "getTeamMembers",
          data: { id: props.id, eventid: props.eventid}
        }
      ).then(async (response) => {
            await response.data.userid.shift()
            setid({ idArray: response.data.userid})
      })
    },[])
  ...
  const submitHandler = async (event) => {
    ...
    await idArray.unshift(props.id)
    axios(
        {
          method: "POST",
          url: USERAPIURL + "updateTeamMembers",
          data: { userid: idArray, eventid: props.eventid }
        }
      ).then(async () => {
          toast.success("Team Members Updated Successfully")
      }).catch(() => toast.error("Couldn't update Team Members"))
    props.close();
  };
          ...
          <div className="inputgrp">
            <div className="inputGrpLeft">
              ...
              <input
                ...
                className="ID_text"
                type="text"
                placeholder="TEX ID of Member 5"
                onChange={(event) => {
                  let idArray = id.idArray
                  idArray[3] = event.target.value
                  setid({ idArray })
                  checkEmpty(3)
                  let newcheckmarks = checkmarks;
                  newcheckmarks[3] = "hide"
                  setCheckMarks(newcheckmarks)
                }}

                value={id.idArray[3]}
              />
              <img src={approved} className={`approvedImage ${checkmarks[3]}`} alt="" />
            </div>
            <button className="checkBtn" onClick={() => {
              checkID(id.idArray[3], 3).then((response) => {
                "OK"
              }
              ).catch((err) => {
                toast.error(err)
